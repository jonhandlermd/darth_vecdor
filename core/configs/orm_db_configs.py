...
odc = pci.bootstrap_import(f"orm_db_configs.py", make_dummy_if_not_exists=True)
...
default_embedder_meta_src_location = odc.default_embedder_meta_src_location

# DEFAULTS AVAILABLE
schema = getattr(odc, 'schema', 'darth_vecdor')
system_schema = getattr(odc, 'system_schema', 't_sys')
vector_size = getattr(odc, 'vector_size', 768*2)
...
dv_query_separator = getattr(odc, 'dv_query_separator', '<dv_query_separator>')
